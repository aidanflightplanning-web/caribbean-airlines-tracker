...
- **ADS-B discovery requires actually being airborne**: a parked aircraft between rotations can keep broadcasting its callsign for hours while sitting at the gate (e.g. overnight, waiting for tomorrow's flight) — without filtering to `on_ground == False`, that gets treated as a "detected" flight even though its real next departure might be many hours away (past the 6-hour staleness cutoff above), producing a bare position-only row with no useful status. On-ground aircraft that genuinely matter (taxiing for imminent departure) are still found via the schedule-based discovery window instead, which is time-aware.
- **CAL's "today vs. yesterday" date lookup is combined, not first-match**: confirmed by testing that CAL's own dept_date indexing doesn't line up cleanly with calendar days — for a real, currently-airborne flight, querying with today's date returned only a far-future "Scheduled" leg (tomorrow's rotation), while the actual "Airborne" leg only showed up under yesterday's date. The lookup now combines legs from both dates instead of stopping at the first one that returns anything, so the real current leg doesn't get missed.
- **No CAL record at all → no row, not "Unknown"**: if CAL genuinely has zero data for a flight number (as opposed to data that's just not in a relevant time window), the flight is dropped from the table entirely rather than shown as a bare "Unknown, ? → ?" row. This also naturally filters out ADS-B detections that were never a real Caribbean Airlines flight number in the first place (e.g. a coincidental callsign match from another operator).
